...
class Trainer:
    ...
    def run_epoch(self, epoch, data_loader, mode):
        ...
        for iter_id, meta in enumerate(data_loader):
            ...
            # 原始的没有这一步哈
            meta['hm'] = meta['hm'].to(device=torch.device('cuda'), non_blocking=True)
            meta['reg'] = meta['reg'].to(device=torch.device('cuda'), non_blocking=True)
            meta['wh'] = meta['wh'].to(device=torch.device('cuda'), non_blocking=True)
            
            
            output, loss, loss_stats = self.run_step(model, meta, mode)
            if mode == 'val':  # TODO: eval
                dets = model.module.head.post_process(output, meta) # 长什么样子的
    ...
    def warm_up(self, data_loader):
        ...
        while cur_iter < num_iters:
            for iter_id, batch in enumerate(data_loader):
                ...
                # 原始的没有这一步啊
                batch['hm'] = batch['hm'].to(device=torch.device('cuda'), non_blocking=True)
                batch['reg'] = batch['reg'].to(device=torch.device('cuda'), non_blocking=True)
                batch['wh'] = batch['wh'].to(device=torch.device('cuda'), non_blocking=True)
