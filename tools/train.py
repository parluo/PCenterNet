...
def parse_args():
    ...
    parser.add_argument('--config', default='config/ceternet_xml_dataset.yml', help='train config file path')
...
def main(args):
    ...
    if args.seed is not None:
        logger.log('Set random s eed to {}'.format(args.seed))
    ...
    if 'resume' in cfg.schedule:
        trainer.resume(cfg)
    elif 'load_model' in cfg.schedule:
...
# 切换loss时要改： 1.切换LOSS类 2.decode里的积分 
# 3.yaml文件 2->16 4.DLAhead、GV1LOSS里的积分数量 
# 5.dfloss, Integral里的通道数c//2
# 6.yaml文件里的.pth文件
# 7.yaml文件里的save_dir
